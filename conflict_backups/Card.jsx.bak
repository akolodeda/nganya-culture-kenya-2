...
<<<<<<< HEAD
      role="button"
      tabIndex={0}
      aria-label={`Play sound for ${name}`}
      onMouseEnter={playSound}
      onMouseLeave={stopSound}
      onKeyDown={(e) => { if(e.key === "Enter") playSound(); }}
      className={`relative cursor-pointer rounded-xl overflow-hidden shadow-xl bg-gradient-to-br ${crewColors[name] || "from-gray-700 to-gray-900"} backdrop-blur-xl border border-white/10`}
        ...
        <div className="flex justify-center mt-3 gap-1 h-5">
          {barDelays.map((delay, i) => (
            <motion.div
              key={i}
<<<<<<< HEAD
              animate={isPlaying ? { scaleY: [0.4, 1.25, 0.6] } : { scaleY: 0.4 }}
              transition={{ duration: 0.35, repeat: isPlaying ? Infinity : 0, delay }}
=======
              animate={
                isPlaying
                  ? { scaleY: [0.4, 1.25, 0.6] }
